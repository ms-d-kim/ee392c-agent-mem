...
**final_v3/carryover_kv_origin** — Headline cross-step carry-over figure
(README Finding #4): the per-step projected-KV working set stacked by the step
that first prefilled each logical object. Regenerate with
`python3 -m analysis.carryover traces/final_v3`. The decomposition is
engine-cache-independent by construction; the coding cache on/off panels
differ only through sampled text.

**final_v3/methodology** — Pipeline diagram from workload fixtures through
gates to the prescriptive tier mapping. Regenerate with
`python3 -m analysis.methodology` (see also `final_v3/methodology.md`).

**final_v3/nsight_phase_kernels** — Auxiliary Nsight Systems profile of one
compaction replay (NVTX phase timeline, kernel-class mix, total CUDA memcpy).
Regenerated by `python3 -m analysis.nsight` from the local
`.nsys-rep`/`.sqlite`, which are too large for git; only the derived
`analysis_out/final_v3/nsight_summary.csv` is tracked. Auxiliary evidence per
DECISIONS §4, not a seventh workload.

### Retention / reuse diagnostics

Supporting diagnostics that slice the same trace data by retention class and
reuse cadence; appendix material rather than headline figures.

**final_v3/lifetime_buckets_by_workload** — Object counts per retention class
(short-term vs medium-term) for the default trace of each workload.

**final_v3/workload_retention_composition** — The same split as percentages
per workload (default traces).

**final_v3/semantic_retention_by_class** — Retention-class composition within
each semantic class across default traces.

**final_v3/reuse_interval_by_workload** — Distribution of time between
successive reads of the same object, by workload.

**final_v3/lifetime_reuse_seconds** — Wall-clock variant of `lifetime_reuse`;
prefer the step-normalized version for cross-workload comparisons.

**final_v3/step_duration_by_workload** — Single-step wall-clock duration
ranges for the default traces (step 0 task-setup spans excluded).

### Presentation-only renders

**final_v3/slide_three_point** and **final_v3/slide_what_we_built** — Slide
graphics for the June 1–3 final presentation, generated by
`analysis/slide_three_point.py` and `analysis/slide_what_we_built.py`. They
restate committed results for presentation; they are not report evidence.
